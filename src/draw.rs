...
pub fn draw_pixel(ctx: &mut RenderContext, p: (u32, u32), color: (u32, u32, u32)) {
    let index = rh_coord_to_buffer_index(p, ctx.viewport);
    ctx.buffer[index as usize] = (color.0 << 16) | (color.1 << 8) | color.2;
}
